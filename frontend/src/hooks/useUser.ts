...
const GROUP_FEEDBACK = 'FeedbackAllowed';
...
const useUser = () => {
  ...
  const [isAllowFeedback, setIsAllowFeedback] = useState(false);
  ...
  useEffect(() => {
    ...
    if (Array.isArray(groups)) {
      ...
      setIsAllowFeedback(groups.some(group =>
        group === GROUP_FEEDBACK || group === GROUP_ADMIN
      ));
    } else {
